...
for f in `cat ${IDS}`; do
	if [ -f ${BASEDIR}/bacteria/${f}_protein.faa ]; then
		...
		elif [ -f ${BASEDIR}/archaea/${f}_protein.faa.gz ]; then
			cp ${BASEDIR}/archaea/${f}_protein.faa.gz ${OUTDIR}/
			gzip -d ${OUTDIR}/${f}_protein.faa.gz
		else
			#echo "$f not found, please specify a valid accession ID"
			#exit -1

			# the code below could download missing genomes from NCBI if you're sure the accession ID is valid
